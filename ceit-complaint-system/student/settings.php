...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        $yearLevel  = $_POST['year_level'] ?? $user['year_level'];
        ...
        if (!$errors) {
            $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, department = ?, year_level = ? WHERE user_id = ?")
                ->execute([$firstName, $lastName, $department, $yearLevel, $userId]);
            $_SESSION['first_name'] = $firstName;
            $_SESSION['last_name'] = $lastName;
            flash_set('success', 'Profile updated.');
            header('Location: /ceit-complaint-system/student/settings.php');
            exit;
        }
    }
...
<div class="panel">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-group"><label>CvSU Email</label><input type="email" value="<?= e($user['email']) ?>" disabled></div>
        <div class="form-row">
            ...
            <div class="form-group"><label>Year Level</label>
                <select name="year_level">
                    <?php foreach ($yearLevels as $y): ?>
                        <option value="<?= e($y) ?>" <?= $user['year_level'] === $y ? 'selected' : '' ?>><?= e($y) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
    ...
    <form method="post">
        <div class="form-group"><label>Current Password</label>
            <div class="password-wrap">
                <input type="password" name="current_password" id="s-cur-pw" required>
                <button type="button" class="password-toggle" data-target="s-cur-pw" aria-label="Show password">Show</button>
            </div>
        </div>
        <div class="form-row">
            <div class="form-group"><label>New Password</label>
                <div class="password-wrap">
                    <input type="password" name="new_password" id="s-new-pw" required minlength="8">
                    <button type="button" class="password-toggle" data-target="s-new-pw" aria-label="Show password">Show</button>
                </div>
            </div>
            <div class="form-group"><label>Confirm New Password</label>
                <div class="password-wrap">
                    <input type="password" name="confirm_password" id="s-confirm-pw" required minlength="8">
                    <button type="button" class="password-toggle" data-target="s-confirm-pw" aria-label="Show password">Show</button>
                </div>
            </div>
        </div>
